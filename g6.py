...
def WindowPrint():
    ...
    [sg.Multiline(default_text=CorrectText,key='-keybox-', size=(30, 20),font="Any 10")],
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED:
            ...
            if b:
                ...
                iC2=cv2.resize(thresholded_image,(420,310))
            else:
                kernel = np.ones((kernel_size, kernel_size), np.uint8)
                ...
                for idx, contorno in enumerate(contornos):
                    ...
                    if len(approx) == 4:
                        ...
                        iC2 = cv2.resize(iC.copy(),(420,310))
                        a=True
                    else:
                        a=False
...
def Opciones():
    ...
    global ocrGLOB, autGLOB, jsonF
    ocrT='GOOGLE OCR'
    autT='AUTOCORRECTOR TESSERACT'
    layout4 = [
    [sg.Text('Opciones de Escaner:')],
    [sg.Text('Modelo OCR:', size=(23, 1)),sg.Radio('GOOGLE CLOUD', 1, key='-OCR-')],
    [sg.Text('Modelo OCR:', size=(23, 1)),sg.Radio('PYTESSERACT',1, key='-OCR-')],
    [sg.Text('Modelo AUTOCORRECTOR:  ', size=(23, 1)),sg.Radio('GOOGLE CLOUD', 2,key='-AUTOCORRECTOR1-')],
    [sg.Text('Modelo AUTOCORRECTOR:  ', size=(23, 1)),sg.Radio('PYAUTOCORRECT',2,key='-AUTOCORRECTOR2-')],
    [sg.Text('Modelo AUTOCORRECTOR:  ', size=(23, 1)),sg.Radio('NO APLICAR',2,key='-AUTOCORRECTOR3-')],
    [sg.Text('Clave para el OCR:  ', size=(23, 1)),sg.Radio('NO APLICAR',2,key='-AUTOCORRECTOR3-')],
    [sg.Text('Cambiar ubicación de clave', size=(23, 1)),sg.Input(enable_events=True, key='-IN-',size=(11, 1)), sg.FilesBrowse()],
    ...
    while True:  # <-- Mueve el while True aquí dentro
            ...
            if evop == sg.WIN_CLOSED or evop == 'Cancelar':
                ...
                if file_path.lower().endswith('.json'):
                    ...
                    window4['-IN-'].update('')
                    sg.popup_error('Por favor, selecciona un archivo JSON.')
    window4.close()            
...
def mainBsc():
    ...
    while True:  # Event Loop
        ...
        #ret, frame = capd.read()
            ...
            if not ret:
                window2.hide()
                sg.popup("no se detectó cámara la camara")
                window2.un_hide()
                break
            ...
            if not ret:
                window2.hide()
                sg.popup("no se detectó la camara Derecha")
                window2.un_hide()
                break
            height, width = img.shape[:2]
            imgCol = cv2.cvtColor(img.copy(), cv2.COLOR_BGR2RGB)
